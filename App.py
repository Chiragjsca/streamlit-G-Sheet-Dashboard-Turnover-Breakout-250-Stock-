...
def LQ(df,symbol_col):
	...
	for(L,M)in I.iterrows():
		...
		for J in I.columns:
			...
			B=J.lower();C,E=D,G
			if JF in B:C,E=f"https://www.tradingview.com/symbols/{A}/",f"Tre {A}"if not B.endswith(H)else G
			elif JG in B:C,E=f"https://www.equitypandit.com/historical-data/{A}",f"History {A}"if not B.endswith(H)else G
			elif JH in B:C,E=f"https://www.screener.in/company/{A}",f"Scr {A}"if not B.endswith(H)else G
			elif JI in B:C,E=f"https://zerodha.com/markets/stocks/NSE/{A}",f"🪁 {A}"if not B.endswith(H)else G
			elif JJ in B:C,E=f"https://chartink.com/stocks-new?load-snapshot=exponential-moving-average-simple-moving-average-simple-moving-average-moving-average-convergence-divergence-chart-snapshot-175&symbol={A}",f"CL {A}"if not B.endswith(H)else G
			elif JK in B:C,E=f"https://marketsmithindia.com/mstool/eval/{A}/evaluation.jsp",f"ms {A}"if not B.endswith(H)else G
			elif JL in B:C,E=f"https://www.nseindia.com/get-quotes/equity?symbol={A}",f"nse📰 {A}"if not B.endswith(H)else G
...
def DJ(df,col_name,st_container,display_label=D):
	...
	if D in df.columns:
		A=df[D].astype(F).str.replace(BH,C,regex=B);A=H.to_numeric(A,errors=AN).replace([A6.inf,-A6.inf],A6.nan);E=A.dropna()
		if not E.empty:
...
def By(val):
	...
	A=re.sub(Dv,C,F(val));return Q(A)
def Gv(df):
	A=df.copy();D=[A for A in A.columns if A.startswith(Aa)or A.startswith(Ab)or A==n];A=A.drop(columns=D,errors='ignore')
	for B in A.select_dtypes(include=['object']).columns:A[B]=A[B].apply(lambda x:re.sub(Dv,C,F(x))if H.notnull(x)else x)
	return A
...
def La():
	...
	G=[A for A in E.columns if not A.startswith(Aa)and not A.startswith(Ab)];I=S((A for A in G if A.lower()in[Cz,Ac,Dw,Dx,C_,Dy]),D);J=S((A for A in G if A4 in A.lower()),D);K=S((A for A in G if D0 in A.lower()or x in A.lower()),D);L=S((A for A in G if Ap in A.lower()),D);M=S((A for A in G if Dz in A.lower()and'face'not in A.lower()and'enterprise'not in A.lower()),D);N=S((A for A in G if Ap in A.lower()),D)
	if not I:return H.DataFrame()
	A=H.DataFrame();A[j]=E[I].astype(F).str.strip();A[AM]=H.to_numeric(E[J].astype(F).str.replace(BH,C,regex=B),errors=AN)if J else i;A[D_]=H.to_numeric(E[K].astype(F).str.replace(BH,C,regex=B),errors=AN)if K else i;A[A2]=H.to_numeric(E[L].astype(F).str.replace(BH,C,regex=B),errors=AN)if L else i;O=A[AM]*A[A2]
	if M:A[CR]=H.to_numeric(E[M].astype(F).str.replace(P,C,regex=B),errors=AN)
	else:A[CR]=O
	if N:A[E0]=H.to_numeric(E[N].astype(F).str.replace(P,C,regex=B),errors=AN)
	else:A[E0]=O
...
def DK(row,actual_cols):
	...
	if G is not D:
		...
		elif G<=Aq:B+=4;A.append(f"🟡 Manageable Debt (D/E={G:.2f})")
...
def Lq():
	...
	try:
		...
		for B in I:
			H=F(B.get(j,C)).strip()
			if H:G[H]={BI:F(B.get(D1,C)),A4:F(B.get(AM,C)),CT:F(B.get(D2,C)),Bj:F(B.get(E4,C)),D3:F(B.get(E5,C))}
...
def Ls(row_data,cols):
	...
	if B and I and J:G=(I-J)/52;A[E6]=b(G,2);A['sl_tight']=b(B-Aq*G,2);A[N]=b(B-1.5*G,2);A['sl_wide']=b(B-2.*G,2);O=2.;H=B-A[N];A['target_1r']=b(B+H*Aq,2);A['target_2r']=b(B+H*O,2);A['target_3r']=b(B+H*3.,2);A[D5]=b(K,2)if K else D;A['trail_sl_200dma']=b(L,2)if L else D;A['risk_pct']=b(H/B*100,2)if B else D
	return A
def Ee(history):
...
if not Eg.empty:
	...
	if H0:Lw=K[R].astype(F).apply(lambda x:x.str.contains(H0,case=J,na=J)).any(axis=1);K=K[Lw]
	...
	for DL in L_:
		M0=sorted([A for A in H1[DL].unique()if F(A).strip()!=C]);H3=A.sidebar.multiselect(f"Filter by {DL}:",options=M0,key=f"filter_cat_{DL}")
	...
	if Cc!=Jc:
		...
		if H4 and H6:
			DM=H.to_numeric(K[H4].astype(F).str.replace(BH,C,regex=B),errors=AN);DN=H.to_numeric(K[H6].astype(F).str.replace(BH,C,regex=B),errors=AN)
			if Cc==Jf:K=K[DM>DN]
			elif Cc==Jg:K=K[DM<DN]
			elif H5:
				DO=H.to_numeric(K[H5].astype(F).str.replace(BH,C,regex=B),errors=AN)
	...
	with A.expander(f"🚀 Executive Dashboard — {y}",expanded=B):
		...
		if BS and BS in W.columns:HK=U(W[BS].astype(F).str.contains('breakout|buy|bullish',case=J,na=J).sum())
		HL=0
		if BT and BT in W.columns:HL=U(W[BT].astype(F).str.contains('buy',case=J,na=J).sum())
		HM,HN=0,0;HO=0
		...
		if n in W.columns:B_=W[n].astype(F).str.strip()
		else:B_=BX.astype(F).str.replace('<[^>]+>',C,regex=B).str.strip()
	...
	if Hb:K=K[K[n].astype(F).str.contains(Hb,case=J,na=J)]
	...
	for p in K.columns:
		...
		if Ha==CW and Q(K)>0:
			Ey=By(K.iloc[0][p]);Ez=Q(F(p));Cg=U(AA(Ey,Ez)*7+22)
			if Cf:Cg+=30
			DX,DY=Cg,40
		elif Ha==CX and Q(K)>1:
			Ey=By(K.iloc[1][p]);Ez=Q(F(p));Cg=U(AA(Ey,Ez)*7+22)
			if Cf:Cg+=30
			DX,DY=Cg,40
	...
	if BY is not D and Q(BY)>0 or Q(K)>0:
		...
		if G:
			...
			with A9[1]:Hf=f"https://charting.nseindia.com/?symbol={G}-EQ";A.markdown(f"**NSE Interactive Chart Frame** &nbsp;|&nbsp; [🌐 Open in Browser]({Hf})",unsafe_allow_html=J);A.caption(Bn);O.html(f'<iframe src="{Hf}" width="100%" height="{A8}" style="border:none; border-radius:5px;"></iframe>',height=A8+20)
			with A9[2]:Hg=f"https://www.equitypandit.com/historical-data/{G.lower()}";A.markdown(f"**EquityPandit Historical Matrix Data** &nbsp;|&nbsp; [🌐 Open in Browser]({Hg})");A.caption(Bn);O.html(f'<iframe src="{Hg}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[3]:Hh=f"https://www.equitypandit.com/share-price/{G.lower()}#chart";A.markdown(f"**Bullish / Bearish Zone Indicator** &nbsp;|&nbsp; [🌐 Open in Browser]({Hh})");A.caption(Bn);O.html(f'<iframe src="{Hh}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[4]:Hi=f"https://www.screener.in/company/{G}/consolidated/";A.markdown(f"**Screener Corporate Filings** &nbsp;|&nbsp; [🌐 Open in Browser]({Hi})");A.caption(Bn);O.html(f'<iframe src="{Hi}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[5]:Hj=f"https://zerodha.com/markets/stocks/NSE/{G}/";A.markdown(f"**Zerodha Markets Financial Performance Metrics** &nbsp;|&nbsp; [🌐 Open in Browser]({Hj})");A.caption(Bn);O.html(f'<iframe src="{Hj}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[6]:Hk=f"https://marketsmithindia.com/mstool/eval/{G.lower()}/evaluation.jsp";A.markdown(f"**MarketSmith India Institutional Trading Evaluation Engine** &nbsp;|&nbsp; [🌐 Open in Browser]({Hk})");A.caption(Bn);O.html(f'<iframe src="{Hk}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[7]:Hl=f"https://www.tradingview.com/symbols/{G}/";A.markdown(f"**TradingView Comprehensive Asset Market Registry Summary Profile** &nbsp;|&nbsp; [🌐 Open in Browser]({Hl})");A.caption(Bn);O.html(f'<iframe src="{Hl}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[8]:
				...
				if not EY:A.warning(Jy)
					...
					if A.button('✨ Generate AI Analysis',use_container_width=B,key='btn_ai_analysis'):
						with A.spinner(f"Analyzing {G} with {DZ}..."):
							try:F0={A:B for(A,B)in T.items()if not F(A).startswith(Bo)};Ci=f"""
...
''';Hn=EZ(Ci,Mf);A.session_state[GI]={DB:G,DC:Hn};A.markdown('### 📋 Your Custom Strategy Code:');A.write('Copy the code below and paste it into the TradingView Pine Editor.');A.markdown(Hn)
							except g as BZ:A.error(f"AI error: {BZ}")
					if A.session_state.get(GI,{}).get(DB)==G:Mh=A.session_state[GI][DC];Mi=Ee([[G,'Pine Script',Hm,Mh,l.now().strftime(CQ)]]);A.download_button('📥 Save Pine Script as Excel',data=Mi,file_name=f"PineScript_{G}_{l.now().strftime(Bk)}.xlsx",mime=Bl,key='dl_pine_excel')
			...
			with A9[10]:
				A.markdown(f"### 🔬 Bottom Fishing Analysis: **{G}**");A.caption('Scores this stock on 8 key criteria for buying from the bottom. Based entirely on your live sheet data.');Ho={A:B for(A,B)in T.items()if not F(A).startswith(Bo)};C0,F2,F3=DK(Ho,R);F4=Ad if C0>=75 else JW if C0>=55 else JX if C0>=35 else AP;A.markdown(f'''
...
""";F6=EZ(Ci,F5);A.session_state['last_bf_ai_result']={DB:G,DC:F6};A.session_state.ai_history.append([G,F5,'Bottom Fishing Deep Analysis',F6,l.now().strftime(CQ)]);A.success('✅ AI Analysis Complete');A.markdown(F6)
							except g as BZ:A.error(f"AI error: {BZ}")
					A.markdown(c);A.markdown('#### 📤 Share BF Score Card');Hp=f"""🔬 *Bottom Fishing Score: {G}*
...
"""+'\n'.join(F3[:5])+f"\n\n🕒 {l.now().strftime(Jz)}\n📌 NSE Stock Dashboard";Ml=urllib.parse.quote(Hp);Mm=urllib.parse.quote(Hp);Mn,Mo=A.columns(2)
			...
			with A9[11]:
				A.markdown(f"### 🎯 GTT Order Calculator: **{G}**");A.caption('Auto-suggest Stop-Loss, Targets & ATR-based GTT levels from your live sheet data.');Mp={A:B for(A,B)in T.items()if not F(A).startswith(Bo)};AF=Ls(Mp,R)
...
🕒 {l.now().strftime(Jz)}""";A.code(F9,language=C);N1=urllib.parse.quote(F9);N2=urllib.parse.quote(F9);N3,N4=A.columns(2)
			...
			with A9[12]:
				A.markdown(f"### 📊 Watchlist Manager");Hw={A:B for(A,B)in T.items()if not F(A).startswith(Bo)};N5,N6,_=DK(Hw,R);N7=F(Hw.get(A7,C))if A7 else C;FA=G in A.session_state.watchlist;A.markdown(f"**Current Stock: {G}** {"✅ Already in Watchlist"if FA else C}");N8=A.text_input('📝 Note (optional):',value=A.session_state.watchlist.get(G,{}).get(BI,C),placeholder='e.g. Near 52W low, watching for breakout',key=f"wl_note_{G}");N9,NA=A.columns(2)
				with N9:
					if A.button(f"{"🔄 Update"if FA else"➕ Add"} {G} to Watchlist",use_container_width=B,key='wl_add_btn'):
						Lr(G,cmp=N7,note=N8,bf_score=F(N5),bf_grade=N6);NB=Ed()
						if NB:A.success(f"✅ {G} saved to Watchlist (Google Sheet updated!)")
						else:A.info(f"✅ {G} added to session Watchlist (Sheet write failed — check secrets).")
						A.rerun()
				with NA:
					if FA:
						if A.button(f"❌ Remove {G} from Watchlist",use_container_width=B,key='wl_rm_btn'):G_(G);Ed();A.rerun()
			...
			with A9[13]:
				A.markdown(f"### 📰 Latest News & Alerts: **{G}**");import urllib.request,urllib.parse,xml.etree.ElementTree as C3,datetime as l,email.utils
				def NJ(pubdate_str):
					try:
			...
			with A9[0]:
				with A.expander(f"🕯️ Price Chart & Technical Indicators — {G}",expanded=B):
					NL=A.select_slider('History range:',options=['3mo','6mo','1y','2y','5y'],value='1y',key=f"chart_period_{G}")
					with A.spinner(f"Loading price history for {G}..."):m=LP(G,period=NL)
					if m.empty or BG not in m.columns:A.warning(f"⚠️ No historical price data available for **{G}** via Yahoo Finance (tried `{G}.NS`). The symbol may be delisted, renamed, or not tracked by Yahoo.")
					else:
						...
						with NO:
							...
							if y!=BF:
								...
								if not Dj.empty:
									...
									if IB:
										IC=Dj[Dj[IB].astype(F).str.strip()==G]
										if not IC.empty:Ng=IC.iloc[0].to_dict();X={A:B for(A,B)in Ng.items()if not F(A).startswith(Aa)and not F(A).startswith(Ab)and F(A)!=n}
							def Y(row,primary_dict,*K):
								def B(r_data):
									...
									H=[A for A in H if not F(A).startswith(Aa)and not F(A).startswith(Ab)and F(A)!=n]
									for L in K:
										I=L.lower().strip()
										for G in H:
				...
				with A.expander(f"📋 {G} — Google Sheet Data",expanded=B):
					...
					if Bb is not D and B9 is not D and 0<B9<Bb:IZ=Bb-B9;FP=B9/Bb*100;Ia=P.Figure(P.Sankey(arrangement=Bu,textfont=E(color=As,size=13,family=Bv),node=E(pad=30,thickness=18,line=E(color=Bw,width=.5),label=[f"Net Sales<br>₹{Bb:,.2f} Cr (100%)",f"Net Profit<br>₹{B9:,.2f} Cr ({FP:.1f}%)",f"Total Expenses<br>₹{IZ:,.2f} Cr ({100-FP:.1f}%)"],color=[AE,w,AP]),link=E(source=[0,0],target=[1,2],value=[B9,IZ],color=['rgba(15,157,88,0.35)','rgba(234,67,53,0.35)'])));Ia.update_layout(title=f"💰 Revenue & Expenses Flow — {G} (Net Margin {FP:.1f}%)",template=q,height=320,margin=E(t=45,b=10,l=10,r=10),font=E(size=12));A.plotly_chart(Ia,use_container_width=B,key=f"sankey_{G}");A.caption('Based on Net Sales / Net Profit from the Fundamentals data above. "Total Expenses" is the remainder (Net Sales − Net Profit) — your sheet doesn\'t carry a Cost-of-Revenue/Opex breakdown, so a multi-stage flow (Gross → Operating → Net) isn\'t available for this stock.')
					elif Bb is not D and B9 is not D:A.info(f"Revenue & Expenses flow needs a normal profitable split (0 < Net Profit < Net Sales). {G} currently shows Net Sales ₹{Bb:,.2f} Cr and Net Profit ₹{B9:,.2f} Cr, which doesn't fit a simple flow diagram (e.g. a net loss).")
					...
					if C9 is not D and C9>0 and FQ:
						...
						else:A.info(f"{G}'s itemized asset categories (₹{Id:,.2f} Cr) add up to more than the reported Total Assets (₹{C9:,.2f} Cr) — likely a data mismatch between sheet rows, so the Asset Deployment flow isn't shown to avoid a misleading chart.")
	...
	def O3():
		...
		if V:
			...
			for(AR,N)in K.iterrows():
				o=F(N.get(n,C)).strip();A5=N.get(A7,C)if A7 else C;A6=f"https://charting.nseindia.com/?symbol={o}-EQ";A8=f'<a href="{A6}" target="_blank" style="text-decoration:none; color:#000000; font-weight:bold;">{o}</a>';L={W:A8,a:A5}
				for(I,A9)in V.items():
				...
				if B5:
					t=F(N.get(B5,C)).replace(AB,C).strip()
					try:L[An]=M(t)if t not in[C,A3,AO]else D
					except AY:L[An]=D
				if B6:
					u=F(N.get(B6,C)).replace(AB,C).strip()
					try:L[Ao]=M(u)if u not in[C,A3,AO]else D
					except AY:L[Ao]=D
				if Ce:L[Bp]=F(N.get(Ce,C)).strip()
				if BS:L[Bq]=F(N.get(BS,C)).strip()
				if DR:L[Br]=F(N.get(DR,C)).strip()
				if DS:L[Bs]=F(N.get(DS,C)).strip()
				if BT:L[Bt]=F(N.get(BT,C)).strip()
				AD={A:B for(A,B)in N.items()if not F(A).startswith(Bo)};AE,AF,_=DK(AD,R);L[b]=AE;L[d]=AF;m.append(L)
			S=H.DataFrame(m)
			...
			for G in E.columns:
				if G in(e,):continue
				if f==CW and Q(E)>0:Y=By(E.iloc[0][G]);Z=Q(F(G));P=U(AA(Y,Z)*7+22)
				elif f==CX and Q(E)>1:Y=By(E.iloc[1][G]);Z=Q(F(G));P=U(AA(Y,Z)*7+22)
				else:AM={W:140,a:130,Am:110,b:110,d:160,BL:100,B2:140,An:110,Ao:110,Bp:120,Bq:130,Br:130,Bs:130,Bt:130};P=AM.get(G,130)
				T=AA(70,min(P,90))
				if G==W:O.configure_column(G,width=P,minWidth=T,pinned=DA,cellRenderer=DW)
				elif G==a:O.configure_column(G,width=P,minWidth=T)
				elif G==b:O.configure_column(G,width=P,minWidth=T,cellStyle=AJ)
				elif G==d:O.configure_column(G,width=P,minWidth=T,cellStyle=AK)
				elif G in(Bp,Bq,Br,Bs,Bt):O.configure_column(G,width=P,minWidth=T,cellStyle=AL)
				elif G==A2:O.configure_column(G,width=P,minWidth=T)
				elif G in V or G==B2:O.configure_column(G,width=P,minWidth=T,cellStyle=AI)
				else:O.configure_column(G,width=P,minWidth=T)
	...
	def O4():
		...
		for(AJ,d)in K.iterrows():
			E={A:B for(A,B)in d.items()if not F(A).startswith(Bo)};e,t,u=DK(E,R)
			if e>=X:
				f=F(d.get(n,C)).strip();v=E.get(A7,C)if A7 else C;g=S((A for A in R if E7 in A.lower()),D);w=E.get(g,C)if g else C;x=f"https://charting.nseindia.com/?symbol={f}-EQ";y=f'<a href="{x}" target="_blank" style="text-decoration:none; color:#000000; font-weight:bold;">{f}</a>';P=D
				if BQ:
					h=F(E.get(BQ,C)).replace(AC,C).replace(AB,C).strip()
					try:P=M(h)if h not in[C,A3,AO]else D
					except AY:P=D
				z=F(E.get(BR,C)).strip()if BR else k;A0=F(E.get(BU,C)).strip()if BU else k;A1=F(E.get(B5,C)).strip()if B5 else k;A2=F(E.get(B6,C)).strip()if B6 else k;A4=F(E.get(Ce,C)).strip()if Ce else k;A5=F(E.get(BS,C)).strip()if BS else k;A6=F(E.get(DR,C)).strip()if DR else k;A8=F(E.get(DS,C)).strip()if DS else k;A9=F(E.get(BT,C)).strip()if BT else k;L.append({j:y,W:e,GM:t,AM:v,BL:z,Am:f"{P:.2f}%"if P is not D else k,B2:A0,An:A1,Ao:A2,Bp:A4,Bq:A5,Br:A6,Bs:A8,Bt:A9,GV:F(w)[:30],m:' | '.join(u[:3])})
		...
		if L:
			...
			for G in I.columns:
				if a==CW and Q(I)>0:Y=By(I.iloc[0][G]);Z=Q(F(G));O=U(AA(Y,Z)*7+22)
				elif a==CX and Q(I)>1:Y=By(I.iloc[1][G]);Z=Q(F(G));O=U(AA(Y,Z)*7+22)
				else:O=AF.get(G,120)
				T=DA if G==j else D;V=AA(70,min(O,90))
				if G==W:N.configure_column(G,width=O,minWidth=V,pinned=T,cellStyle=AD)
				elif G==j:N.configure_column(G,width=O,minWidth=V,pinned=T,cellRenderer=DW)
				elif G in(Bp,Bq,Br,Bs,Bt):N.configure_column(G,width=O,minWidth=V,pinned=T,cellStyle=AE)
				else:N.configure_column(G,width=O,minWidth=V,pinned=T)
	...
	O4()
	if AJ:
		A.markdown(c);A.markdown('### 🏆 Top 10 & Bottom 10 Performers (Daily badges)');CG=K.copy();CG[AJ]=H.to_numeric(CG[AJ].astype(F).str.replace(BH,C,regex=B),errors=AN);CG=CG.dropna(subset=[AJ]);O5=CG.nlargest(10,AJ);O6=CG.nsmallest(10,AJ);AU,AV=A.columns(2)
		with AU:
			Ir="<h4 style='margin-top:0px; margin-bottom:8px;'>⬆️ Top 10 (Daily)</h4>"
			for(_,Bf)in O5.iterrows():
				Cq=F(Bf.get(n,C)).strip();AG=Bf[AJ];Cr=Bf.get(A7,C)if A7 else C
				try:Bg=M(F(Cr).replace(AB,C).strip());FY=M(AG);FZ=Bg/(1+FY/100);Fa=Bg-FZ;Cs=f"<span style='font-size: 0.85em; opacity: 0.75; margin-right: 6px;'>+{Fa:,.2f}</span>";Ct=f"₹{Bg:,.2f}"
				except:Ct=f"₹{Cr}";Cs=C
...
try:
	...
	if Q(CH)>0:
		...
		with A.spinner('Scanning Top 30 stocks for Circuit & 52-Week Breakouts (15 Days)...'):
			for G in It:
				A0=F(G).strip();BC=O7(A0,limit=15)
		...
		with OE:
			OS=A.columns(2);Fc=0
			for A0 in[F(A).strip()for A in It]:
		...
		with OF:
			...
			for Cv in CH[:10]:
				A0=F(Cv).strip();BC=O8(A0,limit=5)
				if BC:
					with OV[Fe%2]:
		...
		with OG:
			...
			for Cv in CH[:10]:
				A0=F(Cv).strip();BC=O9(A0,limit=6)
				if BC:
					with OW[Ff%2]:
		...
		with OH:
			...
			for Cv in CH[:15]:
				A0=F(Cv).strip();Fi=OA(A0,limit=7)
				if Fi:
					with OY[Fh%2]:
		...
		with OI:
			...
			with Oa:Iy=[F(A).strip()for A in CH[:60]];Iz=A.multiselect('🔍 Stocks to view:',options=Iy,default=Iy[:4],key='doc_hub_stocks_v2')
